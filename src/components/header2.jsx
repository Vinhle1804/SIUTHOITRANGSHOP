...
function Header2() {
  ...
  return (
    <>
      <header
        className={`fixed w-full top-0 z-10 transition-all duration-300 ${isSmall ? 'bg-white text-black py-3' : 'bg-white text-black py-4'}`}
      >
        <div className={`container mx-auto px-6 flex items-center ${isSmall ? 'justify-between' : 'justify-between'}`}>
          <div className={`flex items-center space-x-4 ${isSmall ? 'justify-center' : ''}`}>
          <a href="/"> 
            <img
            src={isSmall ? '/utils/logo-black-2_png.webp' : '/utils/logo-black-2_png.webp'}
            alt="Logo"
            className={`h-10 ${isSmall ? 'mx-auto' : ''}`}
          />
            </a>
            <ul className={`flex space-x-6 text-lg font-medium ${isSmall ? 'mx-4' : ''}`}>
          ...
          <div className="flex flex-col overflow-y-auto max-h-[400px]">
            {cart.map((item, index) => (
              <div key={index} className="flex items-center justify-between mb-4 p-4 bg-white shadow-sm">
                ...
                <div className="w-1/6">
                  <img className="h-16 w-16 object-cover" src={item.imageSrc} alt={item.name} />
                </div>
                <div className="w-2/6">
